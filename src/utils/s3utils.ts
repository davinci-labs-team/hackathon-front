...
export const generateSignedUrls = async (bucketName: string, filePaths: string[]): Promise<string[]> => {
  if (!filePaths || filePaths.length === 0) return []

  try {
    const urls = await Promise.all(
      filePaths.map((filePath) => S3BucketService.getFileUrl(bucketName, filePath))
    )
